...
Ontario bridge data in React.  In so doing, we'll learn about differences between Vue and
...
From this point onward, React will control the contents of this portion of our page, updating it as necessary in response to changes in our data.
...
that we haven't used `scoped` here, and instead use `className`.  As discussed above,
...
```jsx
<MenuItem onClick={() => this.props.onChange(bridge) ...>
```
